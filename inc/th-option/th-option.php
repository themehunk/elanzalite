...
function menu_tab() {
    $menu_title = esc_html__('Elanzalite Options', 'elanzalite');
    add_theme_page( esc_html__( 'Elanzalite', 'elanzalite' ), $menu_title, 'edit_theme_options', 'thunk_started',array($this,'tab_page'));
...
wp_enqueue_script('elanzalite-admin-load', get_template_directory_uri() . '/inc/th-option/assets/js/th-options.js',array( 'jquery', 'updates' ),'1', true);
...
$data = apply_filters(
    ...
    wp_localize_script( 'elanzalite-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('ThemeHunk','elanzalite'),
    'theme_brand_url' => esc_url($theme_data->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'elanzalite'), esc_html__($theme_data->get( 'Name' )), $theme_data->get( 'Version' ) ),
    'welcome_desc' => esc_html__($theme_data->get( 'Name' ).' is a powerful theme made with love for Magazine, Newspaper and Personal blog.', 'elanzalite' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','elanzalite'),
       'post_content' => '',
       'post_status' => 'publish',
  ...
  return ($fid == $post_id)?true:false;
     ...
     /*
      ...
      wp_send_json_success(
        array(
          'success' => true,
          'message' => __( 'Plugin Successfully Activated', 'elanzalite' ),
        )
      );
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'elanzalite' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'elanzalite' );
            ...
            if ( ! is_plugin_active( $plugin_init ) ){
                    ...
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'elanzalite' );
                    }
                }
...
$boj = new elanzalite_theme_option(); ?>